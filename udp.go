	...
	for {
		...
		if peer.IP.Equal(clientIp) {
			...
			if err != nil || bytesParsed == 0 {
				fmt.Println("[ERROR] Cannot parse UDP address", err.Error())
				tcp.Close()
				return
			}
			...
			if err != nil {
				fmt.Println("[ERROR] Cannot forward UDP data to target host", err.Error())
				tcp.Close()
				return
			}
			...
			if err != nil {
				fmt.Println("[ERROR] Cannot forward UDP data to client", err.Error())
				tcp.Close()
				return
			}
		}
...
func handleUdp(tcp *net.TCPConn, buffer []byte) {
	...
	if err != nil {
		fmt.Println("[ERROR] Cannot create UDP connection", err.Error())
